...
def main_svr(Stock, folder):
    ...
    # plt.title(f"{Stock} - SVR Residuals", fontsize=26)
    ...
    Latex = f"SVR & ${train_r2:.3f}$ & ${test_r2:.3f}$ & ${train_mae:.3f}$ & ${test_mae:.3f}$ & ${train_mse:.3f}$ & ${test_mse:.3f}$ & ${train_rmse:.3f}$ & ${test_rmse:.3f}$ & ${time_per_trial:.3f}$ \\\\"
